use bake_deep::{algorithm::{Dqn, dqn::ValueLoss}, approximator::{ComposedQFunction, encoder::MLPEncoder, head::LinearQHead}, buffer::ReplayBuffer, env::CartPole, exploration::{Boltzmann, Exploration}, types::{Logger, Tape}};
...
pub fn main() {
    ...
    let mut exploration = Boltzmann::new(4.0f32);
    ...
    for episode in 0..=4000 {
        ...
        *exploration.temp_mut() = (exploration.temp() * 0.995).max(0.05);
        if episode % 10 == 0 {
            ...
            println!("{episode} {count} {steps} {loss} {q_mean} {td_error} {temp}");
            if episode % 100 == 0 {
                eprintln!("Episode: {episode} Total steps: {count} Steps: {steps} Loss: {loss} Q-Mean: {q_mean} TD-error: {td_error} Temp: {temp}");
            }
            logger.clear();
        }
    }
