use bake_deep::{agent::DoubleDQNAgent, buffer::ReplayBuffer, encoder::MLPEncoder, env::CartPole, head::{LinearQHead}, network::SequentialQNetwork, policy::EpsGreedy, types::Tape};
    ...
    for episode in 0..=4000 {
        ...
        *policy.eps_mut() = (policy.eps() * 0.999).max(0.05);
        if episode % 10 == 0 {
            ...
            let q_mean = q_mean.into_scalar::<f32>();
            let td_error = td_error.mean().into_scalar::<f32>();
            let eps = policy.eps();
