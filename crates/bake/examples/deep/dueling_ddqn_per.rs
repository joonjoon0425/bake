...
use bake_deep::{algorithm::{DoubleDqn, dqn::ValueLoss}, approximator::ConstrainedDuelingQNet, buffer::PrioritizedExperienceReplayBuffer, env::CartPole, exploration::{EpsGreedy, Exploration}, network::MlpDuelingQNet, types::{Logger, Tape}};
use burn::{module::Module, nn::{activation::ActivationConfig::Relu}, optim::AdamConfig, tensor::Device};

pub fn main() {
    println!("count,ep_reward_average,loss,td_error,qmean,eps,beta");
    ...
    let beta0 = 0.4;
    let mut buffer = PrioritizedExperienceReplayBuffer::new(seed, 10000, 0.3, beta0, 1.0.into());
    ...
    for count in 0..=total_steps {
        ...
        if count % 5000 == 0 {
            ...
            println!("{count},{ep_reward_average},{loss},{td_error},{qmean},{},{}", exploration.eps(), buffer.beta());
            logger.clear();
        }
