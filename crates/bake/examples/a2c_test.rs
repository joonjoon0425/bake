use bake_deep::{algorithm::{a2c::A2C, advantage_enum::Advantage, loss_enum::Loss}, buffer::RolloutBuffer, contract::ActorCritic, env::{CartPole, Tape}, logger::MovingAvgLogger, net::basic::MlpSeparatedActorCriticNet, wrapper::ActorCriticWrapper};
...
pub fn main() {
    ...
    let state = A2C { gamma: 0.99, advantage: Advantage::Gae { lambda: 0.96 }, loss_fn: Loss::MseLoss };
    let mut env = CartPole::new(seed, &device);
    let mut actor_critic = ActorCriticWrapper::new(MlpSeparatedActorCriticNet::new(&[4, 128, 2], Relu, &autodiff_device));
