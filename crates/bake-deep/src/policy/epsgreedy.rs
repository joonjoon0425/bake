...
impl EpsGreedy {
    ...
    pub fn sample(&mut self, qvalues: Tensor<2>, barrier: Option<DiscreteMask>) -> Tensor<1, Int> {
        if self.rng.random_range(0.0..1.0) < self.eps {
            let random = Tensor::random_like(&qvalues, Distribution::Default);
            return match barrier {
                Some(mask) => mask.apply(random, -1f32).argmax(1).squeeze_dim(0),
                None => random.argmax(1).squeeze_dim(0)
            };
        } else {
            return match barrier {
                Some(mask) => mask.apply(qvalues, -1e9).argmax(1).squeeze_dim(0),
                None => qvalues.argmax(1).squeeze_dim(0)
            };
        }
