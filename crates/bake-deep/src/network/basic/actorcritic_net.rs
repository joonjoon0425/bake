...
use crate::{exploration::{NoiseReset, NoisyLinear}, network::{ActorCriticNet, EncoderType::{self, Separated}, basic::{Mlp, NoisyMlp}}};
...
/// A Simple Noisy MLP Separated ActorCritic
#[derive(Module, Debug)]
pub struct NoisyMlpActorCriticNet {
    actor_mlp: NoisyMlp,
    critic_mlp: NoisyMlp,
    actor_layer: NoisyLinear,
    critic_layer: NoisyLinear,
}

impl NoisyMlpActorCriticNet {
    /// create a new NoisyMlpActorCriticNet struct with given dimensions and activation unit
    pub fn new(dims: &[usize], activation: ActivationConfig, device: &Device) -> Self {
        if dims.len() < 2 { panic!("NoisyMlpActorCriticNet requires at least two dims: input dimension and output dimension."); }
        let actor_mlp = NoisyMlp::new(&dims[..dims.len() - 1], activation.clone(), device);
        let critic_mlp = NoisyMlp::new(&dims[..dims.len() - 1], activation, device);

        let actor_layer = NoisyLinear::new(dims[dims.len() - 2], dims[dims.len() - 1], device);
        let critic_layer = NoisyLinear::new(dims[dims.len() - 2], 1, device);
...
impl ActorCriticNet for NoisyMlpActorCriticNet {
    type Obs = Tensor<2>;
    type Params = Tensor<2>;
...
impl NoiseReset for NoisyMlpActorCriticNet {
    fn reset_noise(&mut self) {
        self.actor_mlp.reset_noise();
        self.critic_mlp.reset_noise();
        self.actor_layer.reset_noise();
        self.critic_layer.reset_noise();
    }
}