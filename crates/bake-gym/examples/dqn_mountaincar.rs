...
pub fn main() {
    ...
    let mut online = DiscreteQNetWrapper::new(MlpDiscreteQNet::new(&[env.obs_shape()[1], 128, 84, env.n_actions()], Relu, &autodiff_device));
    ...
    // evaluation
    let env = GymMountainCar::new(seed, &device, KwArgs::new().add("render_mode", "human"));
    let mut tape = Tape::new(env);
