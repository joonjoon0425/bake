...
{
    pub fn new(gamma: f32, entropy_coeff: f32, lr_a: f64, lr_c: f64, device: B::Device, actor: EncoderHead<B, E1, H1, 2>, critic: EncoderHead<B, E2, H2, 2>, actor_optimizer: O1, critic_optimizer: O2) -> Self {
        Self {
            gamma,
            device,
            actor,
    ...
    pub fn update(mut self, last_obs: E1::Obs, transitions: BatchedTransition<B, <E1::Obs as Batchable>::Batched<B>, <i64 as Batchable>::Batched<B>, ()>) 
    -> (Self, f32, f32) {
        ...
        returns[n] = if terminated[n - 1] == 1f32 { 0f32 } else { self.critic.forward_single(last_obs, &self.device).detach().squeeze_dim::<1>(1).into_scalar().elem() };

        for t in (0..n).rev() {
            returns[t] = rewards[t] + self.gamma * returns[t + 1] * (1.0 - terminated[t]);
        }
        // 1. advantage
        returns.truncate(n);
